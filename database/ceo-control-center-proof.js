 ...
 *       staff, role clamp [1,7], self-lockout, self-deactivate, self-supervise.
...
(async()=>{
  ...
  // ---- org for company 'co': CEO(7), PM(6), Foreman(3), Worker(2); plus an outsider in 'other' and a Sparks staffer
  for (const p of [
    {id:'ceo',  name:' Celia CEO', pin:'7001', rt:'CEO',         rl:7, sup:null,  co:'co'},
    {id:'pm',   name:'Pat PM',     pin:'7002', rt:'Project Mgr', rl:6, sup:'ceo', co:'co'},
    ...
    {id:'spark',name:'Sam Sparks', pin:'7006', rt:'Support',     rl:6, sup:null,  co:'co'},
  ...
  const ceo   = {person_id:'ceo', role_level:7, company_id:'co', sparks_role:null};
  const pm    = {person_id:'pm',  role_level:6, company_id:'co', sparks_role:null};
  const row = async (id)=> (await q('SELECT * FROM people WHERE id=$1',[id])).rows[0];

  console.log('\n— (A) sanitizePersonChange — the wall —');
  // ALLOW: CEO promotes foreman 3 -> superintendent 5
  const promo = sanitizePersonChange(ceo, await row('fore'), {role_level:5, role_title:'Superintendent'});
  check('CEO promotes foreman to superintendent (role 5) — allowed', promo.role_level===5 && promo.role_title==='Superintendent');
  // CLAMP: role_level 9 -> 7
  check('role_level 9 is clamped to 7 (company cap)', sanitizePersonChange(ceo, await row('fore'), {role_level:9}).role_level===7);
  check('role_level 0 is clamped to 1 (floor)', sanitizePersonChange(ceo, await row('fore'), {role_level:0}).role_level===1);
  // DENY edges
  check('non-CEO (PM role 6) is denied (NOT_CEO)',                 denyCode(()=>sanitizePersonChange(pm,  ()=>{}, {role_level:5}))==='NOT_CEO');
  ...
  check('CEO reaching into another company denied (CROSS_COMPANY)', denyCode(()=>sanitizePersonChange(ceo, {id:'out',company_id:'other'}, {role_level:5}))==='CROSS_COMPANY');
  check('CEO lowering OWN role below 7 denied (SELF_LOCKOUT)',      denyCode(()=>sanitizePersonChange(ceo, {id:'ceo',company_id:'co'}, {role_level:6}))==='SELF_LOCKOUT');
  ...
  check('missing target denied (NO_TARGET)',                       denyCode(()=>sanitizePersonChange(ceo, null, {role_level:5}))==='NO_TARGET');
  // FAIL-CLOSED edges (Codex review) — the bugs that the hand-built-actor proof had masked:
  check('actor with NO company_id denied (NO_COMPANY, fail-closed)', denyCode(()=>sanitizePersonChange({person_id:'ceo',role_level:7,sparks_role:null}, {id:'fore',company_id:'co'}, {role_level:5}))==='NO_COMPANY');
  check('target with NULL company_id denied (CROSS_COMPANY, fail-closed)', denyCode(()=>sanitizePersonChange(ceo, {id:'orphan',company_id:null}, {role_level:5}))==='CROSS_COMPANY');
  // getActor MUST surface company_id (Codex HIGH #1) — without it the route's cross-company guard never fires.
  check('getActor() surfaces company_id from req.auth (the real route source)', getActor({auth:{person_id:'ceo',role_level:7,company_id:'co',sparks_role:null}}).company_id==='co');
  ...
  const applied = sanitizePersonChange(ceo, await row('fore'), {role_level:5, status:'active'});
  await DB.people.update('fore', applied);
  const after = await row('fore');
  check('db.people.update applied the CEO role change (foreman -> 5)', after.role_level===5 && after.status==='active');
  ...
  try {
    ...
    // PM (role 6, is_admin TRUE because polluted) editing SELF must NOT keep role_level/is_admin/status
    const pmSelf = stripPrivilegedFields({person_id:'pm', sparks_role:null, is_admin:true}, 'pm', {role_level:7, is_admin:1, status:'inactive', name:'Pat'});
    ...
    const custOther = stripPrivilegedFields({person_id:'ceo', sparks_role:null}, 'fore', {role_level:5, sparks_role:'admin', company_id:'other'});
    check('customer editing another keeps role_level, drops cross-tenant fields', custOther.role_level===5 && custOther.sparks_role===undefined && custOther.company_id===undefined);
    // Sparks staff are unrestricted
    const sparkAny = stripPrivilegedFields({person_id:'s', sparks_role:'admin'}, 'x', {sparks_role:'support', company_id:'co', role_level:7});
    check('Sparks staff write is unrestricted', sparkAny.sparks_role==='support' && sparkAny.company_id==='co' && sparkAny.role_level===7);
  } catch (e) {
  ...
  try {
    ...
    check('CEO with matching company -> NEXT', run({person_id:'ceo',role_level:7,company_id:'co',sparks_role:null}, 'co')==='NEXT');
    check('Sparks staff -> 403 (denied from CEO window)',   run({person_id:'s',role_level:7,company_id:'co',sparks_role:'admin'}, 'co')===403);
    check('non-CEO (role 6) -> 403',                        run({person_id:'pm',role_level:6,company_id:'co',sparks_role:null}, 'co')===403);
    check('CEO with MISMATCHED route company -> 403',       run({person_id:'ceo',role_level:7,company_id:'co',sparks_role:null}, 'other')===403);
    check('CEO with NULL route company -> 403 (fail-closed)',run({person_id:'ceo',role_level:7,company_id:'co',sparks_role:null}, null)===403);
    check('CEO with NO actor company -> 403 (fail-closed)', run({person_id:'ceo',role_level:7,company_id:null,sparks_role:null}, 'co')===403);
  } catch (e) {
