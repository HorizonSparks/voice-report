...
router.get('/', requireAuth, async (req, res) => {
  try {
    const templates = (await (req.db || DB).templates.getAll()).map(t => ({
...
router.get('/:id', requireAuth, async (req, res) => {
  try {
    const t = await (req.db || DB).templates.getById(req.params.id);
...
router.post('/', requireAdmin, async (req, res) => {
  try {
    ...
    await (req.db || DB).templates.create(t);
...
router.put('/:id', requireAdmin, async (req, res) => {
  try {
    const result = await (req.db || DB).templates.update(req.params.id, req.body);
...
router.delete('/:id', requireAdmin, async (req, res) => {
  try {
    const result = await (req.db || DB).templates.deleteTemplate(req.params.id);
    if (result.error) return res.status(403).json(result);
    res.json(result);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
